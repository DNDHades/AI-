 ...
 */
@ActivityScope
public class Mine_Menu2Presenter extends BasePresenter<Mine_Menu2Contract.Model, Mine_Menu2Contract.View> {
    @Inject
    ...
    @Inject
    public Mine_Menu2Presenter(Mine_Menu2Contract.Model model, Mine_Menu2Contract.View rootView) {
    ...
    public void loadData() {
        mModel.loadData()
                ...
                .subscribe(new ErrorHandleSubscriber<MineMenu2HqsjRsp>(mErrorHandler) {
                    @Override
                    public void onNext(MineMenu2HqsjRsp data) {
    ...
    public void loadPostData() {
        mModel.loadPostData()
                ...
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))//使用 Rxlifecycle,使 Disposable 和 Activity 一起销毁
                .subscribe(new ErrorHandleSubscriber<MineMenu2TjRsp>(mErrorHandler) {
                    @Override
                    public void onNext(MineMenu2TjRsp data) {
                        mRootView.loadPostDataComplete(data);
                    }
                });
    }
}
