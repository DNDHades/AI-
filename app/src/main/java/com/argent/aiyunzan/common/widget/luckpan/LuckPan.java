package com.argent.aiyunzan.common.widget.luckpan;
...
public class LuckPan extends View {
    ...
    private Paint mPaintItemStr2;//转盘文字画笔
    ...
    private float mTextSize = 12;//文字大小
    private float mTextSize2 = 14;//文字大小
    private float mText1TopOffset = -10;//文字1与上边的距离
    private float mText2TopOffset = 3;//文字2与上边的距离
    ...
    private void init() {
        ...
        mPaintItemStr2 = new Paint(Paint.ANTI_ALIAS_FLAG);
        mPaintItemStr2.setColor(Color.parseColor("#ED2F2F"));
        mPaintItemStr2.setStrokeWidth(3);
        mPaintItemStr2.setTextAlign(Paint.Align.CENTER);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        rectFStr = new RectF(-mRadius / 7 * 5, -mRadius / 7 * 5, mRadius / 7 * 5,
                mRadius / 7 * 5);
        //每一个Item的角度
        mItemAnge = 360 / mItemStrs.length;
//        mTextSize = mRadius / 9;
//        mTextSize2 = mRadius / 9;
        mPaintItemStr.setTextSize(ArmsUtils.sp2px(getContext(), mTextSize));
        mPaintItemStr2.setTextSize(ArmsUtils.sp2px(getContext(), mTextSize2));
//        mPaintItemStr.setTextSize(18);// TODO: 2020/4/3
        //数据初始化
        mOffsetAngle = 0;
        mStartAngle = 0;
...
//        mStartAngle -= mRepeatCount * 360 - v;
    ...
    private void drawText(Canvas canvas) {
        for (int x = 0; x < mItemStrs.length; x++) {
            ...
            String[] mItemStrs1 = {"现金", "现金", "现金", "现金", "现金",
                    "现金", "现金", "现金"};
            canvas.drawTextOnPath(mItemStrs1[x], path, 0,
                    ArmsUtils.dip2px(getContext(), mText1TopOffset), mPaintItemStr2);
            canvas.drawTextOnPath(mItemStrs[x], path, 0,
                    ArmsUtils.dip2px(getContext(), mText2TopOffset), mPaintItemStr);
        }
    }
    ...
    private void drawPanItem(Canvas canvas) {
        ...
        for (int x = 1; x <= mItemStrs.length; x++) {
            if (x % 2 == 1) {
                //是奇数
                mPaintArc.setColor(Color.parseColor("#FCDCEC"));
            } else {
                //偶数
                mPaintArc.setColor(Color.parseColor("#F596B6"));
            }
            Path path = new Path();
