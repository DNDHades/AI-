 ...
 */
@ActivityScope
public class LoginAuthCodePresenter extends BasePresenter<LoginAuthCodeContract.Model, LoginAuthCodeContract.View> {
    @Inject
    ...
    @Inject
    public LoginAuthCodePresenter(LoginAuthCodeContract.Model model, LoginAuthCodeContract.View rootView) {
    ...
    public void loadAuthLoginCode(String et_mobile, String et_verifi) {
        mModel.loadAuthLoginCode(et_mobile, et_verifi)
                .subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 2))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .doOnSubscribe(disposable -> {
                    mRootView.showLoading();//显示下拉刷新的进度条
                ...
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))//使用 Rxlifecycle,使 Disposable 和 Activity 一起销毁
                .subscribe(new ErrorHandleSubscriber<LoginAuthLoginCodeRsp>(mErrorHandler) {
                    @Override
                    public void onNext(LoginAuthLoginCodeRsp data) {
                        ArmsUtils.makeText(mApplication, data.getMsg());
